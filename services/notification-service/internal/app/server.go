...
func registerServers(lc fx.Lifecycle, handler *grpcdelivery.NotificationHandler, cfg *config.Config, log *zap.Logger) {
	...
	grpcServer, metricsServer, err := server.StartService(server.ServiceConfig{
		...
		notificationv1.RegisterNotificationServiceServer(s, handler)
	}, interceptors, log)
	if err != nil {
