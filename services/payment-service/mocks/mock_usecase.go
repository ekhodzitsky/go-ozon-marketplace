...
func (m *MockPaymentUsecase) Refund(ctx context.Context, paymentID uuid.UUID, amountCents int64, idempotencyKey string) (*domain.Payment, *domain.Refund, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "Refund", ctx, paymentID, amountCents, idempotencyKey)
	ret0, _ := ret[0].(*domain.Payment)
...
func (mr *MockPaymentUsecaseMockRecorder) Refund(ctx, paymentID, amountCents, idempotencyKey any) *gomock.Call {
	mr.mock.ctrl.T.Helper()
	return mr.mock.ctrl.RecordCallWithMethodType(mr.mock, "Refund", reflect.TypeOf((*MockPaymentUsecase)(nil).Refund), ctx, paymentID, amountCents, idempotencyKey)
}
